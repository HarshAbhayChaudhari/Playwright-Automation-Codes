...
test('PR Save Setting Action',async ({browser})=>
{
    let webContext1;
    ...
    await page.frameLocator('#mfe').locator(inputdata.SaveSettings).click();

    //await page.frameLocator('#mfe').locator("[data-testid='lui-ccl-empty-state-heading']").textContent(); //its shows text(its showing empty)
    //await page.frameLocator('#mfe').locator("text=Please add columns to 'In view' panel in settings to see the content here").textContent();
    //await page.pause();
})