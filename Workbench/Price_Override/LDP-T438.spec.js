...
test('Verify the Override modal form validations',async ({browser})=>
{
    let webContext1;
    ...
    await git.click();
    //await page.pause();
    await frame.locator("input[type='text'] >> nth=2").waitFor(); //select the 1st checkbox in tree view (All from product and location heirarchy)
    ...
    await frame.locator("[placeholder='Please enter override value']").type('2', {delay:100});
    await frame.locator("text=Current Price$5.19 – $1.63KProposed Price$1.99 – $1.63KCalculated New Price$2.03").textContent();
